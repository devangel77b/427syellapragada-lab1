...
FIX. Statistical analyses were performed in R \cite{R} using the \texttt{dplyr} and \texttt{ggplot2} libraries \cite{dplyr, ggplot2}. 
...
\section{Results}
\Cref{tab:1} and \cref{tab:2} give stuff. \Cref{fig:1} shows stuff.
...
There were several possible sources of error that contributed to differences between the theoretical and experimental results. The most prominent source was likely air resistance, which slowed the objects slightly and prevented them from truly being in free-fall motion. Another likely source of error is human variation. Systematic errors, such as chances of improper calibration, or random errors, like human errors leading to timing inaccuracies \cite{hetzler:2008:reliability,faux:2019:manual}, variation in drop height, and changes in release technique all are possible sources of error. In an attempt to minimize this effect, we recorded at 240fps, which improved precision slightly by providing a greater number or frames and providing frames at a slower rate, however inconsistencies may have still been introduced.
